...
const styleLayouts = {
  accordionLayout:
    "flex flex-col items-start w-full bg-gray-900 opacity-80 h-[17rem] max-h-[60vh] overflow-y-scroll dropdown-container transition delay-1000 duration-500 ease-out",
  dropdownLayout:
    "absolute mt-2 p-1 gap-1 bg-gray-900 text-gray-400 text-sm border-2 border-gray-300 rounded-lg z-50 cursor-pointer",
};
...
const Dropdown = ({ variant, buttonStyle, itemStyle, id }: DropdownProps) => {
  ...
  const { selectedTuning, setSelectedTuning, isCustom } = useTuningStore();
  ...
  const dropdownMaps = {
    instrument: {
      items: instrumentData.map((instrument) => ({
        id: instrument.id,
        title: instrument.title,
      })),
      selectedItem: selectedInstrument
        ? { id: selectedInstrument.id, title: selectedInstrument.title }
        : null,
    ...
    scale: {
      ...
      selectedItem: selectedScale
        ? { id: selectedScale.id, title: selectedScale.title }
        : null,
      isOpen: scaleOpen,
    ...
    tonic: {
      ...
      selectedItem: selectedTonic
        ? {
            id: selectedTonic.id,
            title:
              typeof selectedTonic.name === "string"
                ? selectedTonic.name
                : showSharp
                ? selectedTonic.name.sharp
                : selectedTonic.name.flat,
          }
        : null,
      isOpen: tonicOpen,
    ...
    timeSignature: {
      items: timeSignatureData.map((timeSignature) => ({
        id: timeSignature.id,
        title: timeSignature.title,
      })),
      selectedItem: selectedTimeSignature
        ? { id: selectedTimeSignature.id, title: selectedTimeSignature.title }
        : null,
    ...
    tuning: {
      ...
      selectedItem: selectedTuning
        ? { id: selectedTuning.id, title: selectedTuning.title }
        : null,
      isOpen: tuningOpen,
      toggleDropdown: toggleTuningOpen,
      handleItemClick: (id: string) => {
        setSelectedTuning(id);
        setHeaderText("tuning", isCustom ? "Custom" : selectedTuning.title);
        setForcePlaceholder(false);
        closeAll();
      },
    },
  ...
  const dropdownFormat = `${
    buttonStyle === "accordion"
      ? styleLayouts.accordionLayout
      : styleLayouts.dropdownLayout
  } ${
  ...
  const addAccordionContent = () => (
    <div className="flex justify-between items-center w-full">
      ...
      {isOpen ? (
        <ChevronUp className="transform transition-transform duration-200 h-3 w-3 text-gray-300 mr-2" />
      ) : (
        <ChevronDown className="transform transition-transform duration-200 h-3 w-3 text-gray-300 mr-2" />
      )}
    </div>
  );

  const headerContent = () => {
    return buttonStyle === "accordion"
      ? addAccordionContent()
      : displayHeaderText;
  };

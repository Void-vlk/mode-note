...
const NoteTextDisplay = ({ stringIndex }: NoteTextDisplayProps) => {
  const { noteNames, showSharp, intervalNames, showIntervals, selectedTonic, selectedScale, stringNotes  } = useCombiStore();
  const stringState = stringNotes[stringIndex] || { currentIndex: 0, openNoteId: 0 };
  const { currentIndex, openNoteId } = stringState;

  const getIntervalIndex = useMemo(() => (index: number): number | null => {
  ...
  const getDisplayedNote = (index: number): string | undefined => {
      ...
      return typeof currentNote.name === 'string'
        ? currentNote.name 
        : showSharp
          ? currentNote.name.sharp : currentNote.name.flat;
    }
  };
  ...
  const getCellStyle = (noteId: number, offset: number): string => {
    ...
    let baseStyle = "flex items-center justify-center w-8 h-8 text-white text-sm font-semibold ";
  
    if (intervalIndex !== null && showIntervals) {
      const intervalColor = intervalNames[intervalIndex]?.color;
      return `${baseStyle} ${intervalColor} rounded-full`;
    }
    if (isSelectedTonic) {
      return `${baseStyle} ${isFirst ? "bg-yellow-600 rounded-md mb-1" : "bg-yellow-600 rounded-full"}`;
    }
    return `${baseStyle} ${isFirst ? "bg-orange-500 rounded-md mb-1" : "bg-green-800 rounded-full"}`;
  };
  ...
  const setScaleNotes = (index: number): string => {
    ...
    return selectedScale.intervals.includes(noteIndex) ? 'visible' : 'invisible';
  }
    ...
    <div className="flex flex-col items-center justify-center gap-2">
      {Array.from({ length: 25 }).map((_, offset) => {
        ...
        return (
          <div
            key={offset}
            className={`${cellStyle} ${scaleNoteOpacity}`}
          >

...
export const useInstrumentStore = create<InstrumentState>((set) => ({
  selectedInstrument: {
    ...instrumentData[5], 
    // Default to 8 string & flipped so strings are in correct order
    stringTunings: [...instrumentData[5].stringTunings].reverse(),
  },
  stringIndex: 0,
...
//   setStringThickness: (thickness) => set({ stringThickness: thickness }),
  ...
  setSelectedInstrument: (instrumentId: string) =>
    set(() => {
      ...
      selectedInstrument.stringTunings.forEach((tuning, index) => {
    ...
    return {
      selectedInstrument: {
        ...selectedInstrument,
        stringTunings: [...selectedInstrument.stringTunings].reverse(),
      },
      stringIndex: 0,
    };
  }),

setStringIndex: (index: number) => 
  set(() => ({ 
    stringIndex: index,
  })),
}));
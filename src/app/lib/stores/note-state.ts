...
import { NoteNameTextProps, noteNames, IntervalProps, intervalNames } from '@/lib/data/note-data'
...
export const useNoteStore = create<NoteState>((set) => ({
  ...
  setCurrentIndex: (stringIndex, newIndex) => {
    set((state) => {
      const currentStringNote = state.stringNotes[stringIndex] || { currentIndex: 0, openNoteId: 0 };
      return {
        stringNotes: {
          ...state.stringNotes,
          [stringIndex]: {
